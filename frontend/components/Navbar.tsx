...
export default function Navbar({ publicKey, onConnect, onDisconnect }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    const lastSeen = typeof window !== "undefined"
      ? window.localStorage.getItem("stellar-indigopay:notifications:lastSeen") || undefined
      : undefined;
    ...
    <nav className="sticky top-0 z-50 bg-white/80 dark:bg-[#0A0A1A]/80 backdrop-blur-xl border-b border-[rgba(99,102,241,0.10)] dark:border-[rgba(129,140,248,0.12)] shadow-sm dark:shadow-none">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col leading-tight">
              <span className="font-display font-bold text-[#0F172A] dark:text-[#E2E8F0] text-base tracking-tight">
                Stellar<span className="text-[#4F46E5] dark:text-[#818CF8]">IndigoPay</span>
              </span>
          ...
          <span className={`hidden md:inline-flex text-[11px] font-semibold px-2.5 py-1 rounded-full ${
            isMainnet
            ...
            <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${isMainnet ? "bg-emerald-500" : "bg-gold-500"} animate-pulse`} />
        ...
        <div className="hidden md:flex items-center gap-1">
          {links.map((l) => {
            const isActive = router.pathname === l.href || (router.pathname.startsWith(l.href + "/") && l.href !== "/");
            return (
              <Link
                key={l.href}
                href={l.href}
        ...
        <div className="flex items-center gap-2">
          ...
          {publicKey ? (
            <>
              <div className="hidden sm:flex items-center gap-2 address-tag">
                ...
                <span className="text-[11px] font-medium">{shortenAddress(publicKey)}</span>
              </div>
              <button
            ...
            <button onClick={onConnect} className="btn-primary text-sm py-2 px-4 shadow-md">
          ...
          >
            <svg className="w-5 h-5 text-[#64748B] dark:text-[#A5B4FC]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
              {mobileMenuOpen ? (
